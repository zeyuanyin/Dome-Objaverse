...
def download_url(item):
    ...
    # print('item:', item)
    tar_path = os.path.join(save_dir, item.split("/")[0], item.split("/")[1]+".tar")
    # print('tar_path:', tar_path)
    if os.path.exists(tar_path):
        # print("File exists: {}".format(item))
        pass
    else:
...
if __name__=="__main__":
    ...
    print("Length of the dataset: {}".format(len(data)))
    p = Pool(n_threads)
    p.map(download_url, data)
    # download_url(data[0])
    
    # for item in ['6/41719', '23/126807']:
    #     download_url(item)
